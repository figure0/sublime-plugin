...
def get_prev_msg(prev_status):
    return next((x for x in re.findall(r'{}'.format(MSG_FROM_PREV_STATUS), prev_status)), None)
    ...
    if is_sub_status(msg):
        ...
        if percentage is not None:
            ...
            if parent_msg is not None:
                ...
                parent_status_updated_msg = handle_main_status(parent_msg.strip(), parent_status_order, sub_progress=int(int(percentage) / 5))
                return common_wrap("{} ({} {}%)".format(parent_status_updated_msg, msg, percentage))
