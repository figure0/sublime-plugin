    ...
    if errors_in_active_file:
        ...
        for s, errors in errors_in_active_file.items():
            ...
            for error_data in errors:
                error_start_point = view.text_point(
                    error_data.get("rows")[0] - 1, error_data.get("cols")[0]
                )
                error_end_point = view.text_point(
                    error_data.get("rows")[0] - 1, error_data.get("cols")[1]
                )
                name = generate_unique_region_name(
                    error_start_point, error_end_point, file_name
                )
                HIGHLIGHTED_REGIONS[file_name].append(
                    {
                        "region": range(error_start_point, error_end_point + 1),
                        "message": suggestion.get("message"),
                        "name": name,
                        "severity": suggestion.get("severity"),
                        "id": suggestion.get("rule"),
                    }
                )
                color = get_highlight_color(suggestion.get("severity"))
                view.add_regions(
                    name,
                    list(map(view.word, range(error_start_point, error_end_point + 1))),
                    color,
                    "",
