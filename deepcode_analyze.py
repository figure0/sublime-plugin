...
def run_analysis(view, on_save=False):
    ...
    add_project_to_initial_analysis_ran_list(project_path)
    ...
    if project_path not in get_consented():
        ...
        if on_save and user_asked_to_consent_for_project(project_path):
            return
        consented = sublime.ok_cancel_dialog(
            "Do you consent for DeepCode to analyzing your project?", "Yes"
        )
...
class DeepcodeAnalyzeCommand(sublime_plugin.WindowCommand):
    def run(self, on_save=False):
        sublime.set_timeout_async(
            lambda: clear_previous_highlight(self.window.active_view()), 10
        )
        sublime.set_timeout_async(
            lambda: run_analysis(self.window.active_view(), on_save), 100
        )
