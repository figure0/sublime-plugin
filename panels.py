...
from .phantoms import add_ingore_commands_phantom, add_see_results_phantom, remove_previous_phantoms
...
def show_results_in_panel(view, resp, link):
    ...
    for k, v in resp.get('files').items():
        ...
        for p, pv in v.items():
            for error in pv:
                ...
                file['issues'].append(get_formated_file_issue(severity, file, error, p, suggestions))
                numberOfIssues += 1
        files_with_issues.append(file)

    
    ISSUES_TEXT_BASE = 'Deepcode: Found problems: {} in {} file(s) '.format(numberOfIssues, numberOfFiles)
    ...
    if numberOfIssues > 0:
        ...
        sublime.set_timeout(lambda: set_status(view, 'DeepCode: No errors found ✅'), 100)
...
def configure_panel(panel, project_path):
    ...
    panel.settings().set("result_file_regex",  r"(?:⚠️|⛔|ⓘ) (.*) \[(\d+), (\d+)\]: (.*) ")
...
def get_formated_file_issue(severity, file, error, suggestion_index, suggestions):
    return {
        'severity': severity,
        'message': '  {} {} [{}, {}]: {}'.format(get_severity_status_string(severity), file['name'], error.get('rows')[0], error.get('cols')[0], suggestions.get(suggestion_index).get('message')),
...
def print_results(panel, files_with_issues, report_message, phantom_counter):
    ...
    files_with_issues.sort(key=lambda k: (k['total'][ERROR], k['total'][WARNING], k['total'][INFO]), reverse=True)
    for record in files_with_issues:
        record['issues'] = sorted(record['issues'], key=lambda k: k['severity'])

    for file in files_with_issues:
        panel.run_command("append", {"characters": '{} \n'.format(get_error_count(file['total'], '{} '.format(file['name'])))})
        for issue in file['issues']:
            panel.run_command("append", {"characters": '{}\n'.format(issue['message'])})
            phantom_counter += 1
            add_ingore_commands_phantom(panel, phantom_counter, file['original_name'], issue)    
