...
def update_highlighted_region(view, point, with_new_line=False):
    target = next((error for error in HIGHLIGHTED_REGIONS[view.file_name()] if point in error["region"]), None)
    (x1, y1), (x2, y2) = view.rowcol(target['region'][0]), view.rowcol(target['region'][-1])
    if with_new_line:
        x1 = x1 + 1
        x2 = x2 + 1
    else:
    ...
    def update_points():
        error_start_point, error_end_point = view.text_point(x1, y1), view.text_point(x2, y2)
...
def insert_new_line_with_comment(view, edit, point, type, id, target):
    ...
    if data[0]:
        ...
        snip = "{0}{1} {2} {3}".format(indent, data[1][0][0], get_ignore_text(type, id), data[1][0][1])
    view.insert(edit, target.begin(), snip)
    view.sel().clear()
    view.sel().add(view.find('<please specify a reason of ignoring this>', target.begin(), sublime.IGNORECASE))
    sublime.set_timeout_async(lambda: update_highlighted_region(view, point, with_new_line=True), 100)
...
def append_to_existing_comment(view, edit, point, type, id, target):
    ...
    snip = "{0}{1}{2}".format(indent, target_text.strip(), get_ignore_text(type, id, append=True))
    update_highlighted_region(view, point)
    view.replace(edit, target, snip)
    view.sel().clear()
    view.sel().add(view.find('<please specify a reason of ignoring this>', target.end(), sublime.IGNORECASE))
    ...
    def run(self, edit, point, type, id):
        if type == 'panel':
            ...
            if does_comment_exist(previous_line_text):
                append_to_existing_comment(self.view, edit, point, type, id, previous_line)
            else:
                insert_new_line_with_comment(self.view, edit, point, type, id, current_line)
