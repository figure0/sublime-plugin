...
def parse_link_data_and_ignore(view, link):
    ...
    view.window().run_command(
        "deepcode_go_to_file_and_ignore",
        {
            "file": link_list[1],
            "type": link_list[0],
            "row": int(link_list[2]),
...
def remove_previous_phantoms(panel, phantom_counter):
    for phantom_index in range(phantom_counter + 1):
        panel.erase_phantoms(
            "{}-{}".format(PANEL_IGNORE_PHANTOM_BASE_NAME, phantom_index)
        )
    phantom_counter = 0
...
def add_settings_key_description_phantom(view, settings_key, description):
    sublime.set_timeout_async(
        lambda: view.add_phantom(
            view.name(),
            view.find(settings_key, 0, sublime.IGNORECASE),
            '<p style="font-size: 12px; color: lightgray; padding: 0; margin: 0">Settings Option - <b style="font-size: 13px; color: snow">{}</b>:</p><p style=" padding: 0; margin: 0 0 20px 0; color: lightblue; font-size: 11px;">{}</p>'.format(
